...
Если harness собрал email для «обработки тикетты», а потом тот же лог скармливается в eval-pipeline для «улучшения модели» — это **сверхцелевая обработка**, нарушение ст. 5 п. 2. Каждый pipeline должен иметь свою цель и своё согласие.
...
- [ ] Каждый pipeline (включая eval) проверен на сверхцелевую обработку
